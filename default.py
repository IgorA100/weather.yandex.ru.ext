...
#IgorA100-START
BASE_URL_CITIES= 'https://pogoda.yandex.ru/static/cities.xml' #публичный урл со списком городов
BASE_URL_W     = 'https://yandex.ru/pogoda'
#IgorA100-END
...
#IgorA100-START
SQL_DIR       = os.path.join(PROFILE, 'sql_data')

#Создаем, при необходимости, каталог где храним полученные данные
if not xbmcvfs.exists(SQL_DIR):
    xbmcvfs.mkdirs(SQL_DIR)

class Weather(object):
    def url_request(self, city_id):
        while True:
            try:
                urllib.urlretrieve('http://export.yandex.ru/weather-ng/forecasts/{}.xml'.format(city_id),
                                           SQL_DIR + '/{}.xml'.format(city_id))
                break
            except:
                continue

    def day_date(self, city_id):
        f = open(SQL_DIR + '/{}.xml'.format(city_id), 'r')
        forecast = f.read()
...
def create_table():
    con = lite.connect(SQL_DIR + '/weather.db')
    with con:
        cur = con.cursor()
    ...
    if not os.path.exists(SQL_DIR + '/cities.db'):
        urllib.urlretrieve('http://weather.yandex.ru/static/cities.xml', SQL_DIR + '/cities.xml')
    tree = ET.parse(SQL_DIR + '/cities.xml')
    root = tree.getroot()
    list_city = []

    for child in root:
        list_city.append(child.attrib['name'])
#        print('-1-->', str(child.attrib['name'].encode("utf8")))
        print('--->', child.attrib['name'])
 #       print('-list_city-->', list_city)
...
#        country_id = input('Выбирете страну из списка и введите название или введите quit для выхода из программы:')
        ...
        if country_id in list_city:
            print('--country in root->', country_id)
